...
import io.github.zenhelix.gradle.testkit.signing.PgpTestUtils.PgpKeyConfig.Companion.pgpKeyConfig
...
public object PgpTestUtils {
    ...
    public class PgpKeyConfig {
        ...
        public companion object {
             ...
             */
            public fun pgpKeyConfig(initializer: PgpKeyConfig.() -> Unit = {}): PgpKeyConfig = PgpKeyConfig().apply(initializer)
        }
    }
    ...
    private fun generateRsaKeyPair(keySize: Int): KeyPair = KeyPairGenerator.getInstance("RSA", BOUNCY_CASTLE_PROVIDER_NAME).apply {
        initialize(keySize, SecureRandom())
    }.generateKeyPair()

    private fun createPgpKeyPair(keyPair: KeyPair): PGPKeyPair = JcaPGPKeyPair(PublicKeyPacket.VERSION_4, PGPPublicKey.RSA_GENERAL, keyPair, Date())

    private fun createKeyRingGenerator(pgpKeyPair: PGPKeyPair, config: PgpKeyConfig, passphrase: String): PGPKeyRingGenerator {
        val algorithm = HashAlgorithmTags.SHA1
        val digestCalculator = JcaPGPDigestCalculatorProviderBuilder().setProvider(BOUNCY_CASTLE_PROVIDER_NAME).build().get(algorithm)
        val contentSignerBuilder = JcaPGPContentSignerBuilder(PGPPublicKey.RSA_GENERAL, algorithm).setProvider(BOUNCY_CASTLE_PROVIDER_NAME)
        val secretKeyEncryptor = JcePBESecretKeyEncryptorBuilder(config.symmetricAlgorithm.id).setProvider(BOUNCY_CASTLE_PROVIDER_NAME).build(passphrase.toCharArray())
