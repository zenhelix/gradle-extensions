...
public class GradleTasksOutputAssert(actual: GradleOutput) :
    ...
    public fun containsExactlyTaskInCategoryInAnyOrder(categoryName: String, vararg taskNames: String): GradleTasksOutputAssert = apply {
        assertThat(actual.categories.map { it.name }).contains(categoryName)
        assertThat(actual.categories.first { it.name == categoryName }.tasks.map { it.name }).containsExactlyInAnyOrder(*taskNames)
    }

    public fun doesNotContainTaskInCategory(categoryName: String, vararg taskNames: String): GradleTasksOutputAssert = apply {
